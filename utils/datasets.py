...
def classification_num_classes(dataset):
    return {"cifar10": 10, "cifar100": 100, "mnist": 10, "imagenet": 1000}.get(
        dataset, None
    )
...
def blobs_get_datasets(dataset_dir=None):
    ...
    if os.path.isdir(dataset_dir):
        X_train, Y_train = torch.load(
            os.path.join(train_dir, "x_data.pth")
        ), torch.load(os.path.join(train_dir, "y_data.pth"))
        X_test, Y_test = torch.load(os.path.join(test_dir, "x_data.pth")), torch.load(
            os.path.join(test_dir, "y_data.pth")
        )
    else:
        X, Y = sklearn_datasets.make_blobs(n_samples=15000, n_features=5, centers=3)
...
class InMemoryImageFolder(datasets.ImageFolder):
    def __init__(self, root, transform):
        ...
        self.inmem = [
            (open(path, "rb").read(), target) for path, target in self.samples
        ]

    def __getitem__(self, index):

        sample_raw, target = self.inmem(index)
        sample = Image.open(io.BytesIO(sample_raw))
...
def imagenet_get_datasets(data_dir, use_data_aug=True):
    ...
    use_inmem = True
    dataset_cls = InMemoryImageFolder if use_inmem else datasets.ImageFolder

    train_dataset = dataset_cls(train_dir, train_transform)
    ...
    test_dataset = dataset_cls(test_dir, test_transform)
